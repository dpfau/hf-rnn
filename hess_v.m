function Rparams = hess_v( x, y, params, v_params, e, g, Je, Jg, He, Hg, dL, ddL )
...
Rdt = zeros(size(h0)); % R_v[dL/dt], where L is the loss
dtt = zeros(size(h0)); % dt at the current time step
for i = size(x,2):-1:1
    Rds = Jg( s(:,i) )' * ( ddL( y(:,i), y_est(:,i) ) .* Ry(:,i) ) + ...
       tprod( Hg( s(:,i) ), [2 1 -1], Rs(:,i), -1 ) * dL( y(:,i), y_est(:,i) ); % R_v[dL/ds]
    RW_yh = RW_yh + Rds * h(:,i)' + ds(:,i) * Rh(:,i);
    Rb_y  = Rb_y  + Rds;
    
    Rdh = ( W_hh' * Rdt + vW_hh' * dtt ) + ( W_yh' * Rds + vW_yh' * ds(:,i) );    
    Rdt = Je( t(:,i) )' * Rdh + tprod( He( t(:,i) ), [2 1 -1], Rt(:,i), -1 ) * ( W_yh' * ds(:,i) + W_hh' * dtt );
    if i > 1
        RW_hh = RW_hh + Rdt * h(:,i-1)' + dt(:,i) * Rh(:,i-1)';
    else
        RW_hh = RW_hh + Rdt * h0' + dt(:,i) * vh0;
    end
    RW_hx = RW_hx + Rdt * x(:,i)';
    Rb_h  = Rb_h + Rdt;
    dtt = dt(:,i);
